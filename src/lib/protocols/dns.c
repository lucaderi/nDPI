...
static void ndpi_check_dns_type(struct ndpi_detection_module_struct *ndpi_struct,
  ...
  case 259:
    ...
    ndpi_set_risk(ndpi_struct, &flow->core, NDPI_DNS_SUSPICIOUS_TRAFFIC, str);
    break;
  }
}
...
static int process_queries(struct ndpi_detection_module_struct *ndpi_struct,
  ...
  for(num = 0; num < dns_header->num_queries; num++) {
    ...
    if(data_len > 253)
      ndpi_set_risk(ndpi_struct, &flow->core, NDPI_MALFORMED_PACKET, "Invalid DNS Query Lenght");
...
#endif
    if(flow->metadata.protos.dns.query_type == 0) {
      /* In case we missed the query packet... */
      flow->metadata.protos.dns.query_type = rsp_type;
    }
...
static int process_answers(struct ndpi_detection_module_struct *ndpi_struct,
  ...
  for(num = 0; num < dns_header->num_answers; num++) {
    ...
    if(rsp_ttl == 0)
      ndpi_set_risk(ndpi_struct, &flow->core, NDPI_MINOR_ISSUES, "DNS Record with zero TTL");
    ...
    if(found == 0) {
      ...
      flow->metadata.protos.dns.rsp_type = rsp_type;
    }
      ...
      if((x + data_len) <= packet->payload_packet_len) {
        ...
        if(rsp_type == 0x05 /* CNAME */) {
          ...
          if((x + ptr_len) <= packet->payload_packet_len) {
            if(found == 0) {
              ...
              ndpi_grab_dns_name(packet, &x,
                                 flow->metadata.protos.dns.ptr_domain_name,
                                 sizeof(flow->metadata.protos.dns.ptr_domain_name), &len,
                                 ignore_checks, NULL);
          ...
          if(found == 0) {
            ...
            if(flow->metadata.protos.dns.num_rsp_addr < MAX_NUM_DNS_RSP_ADDRESSES) {
              /* Necessary for IP address comparison */
              memset(&flow->metadata.protos.dns.rsp_addr[flow->metadata.protos.dns.num_rsp_addr], 0, sizeof(ndpi_ip_addr_t));

              memcpy(&flow->metadata.protos.dns.rsp_addr[flow->metadata.protos.dns.num_rsp_addr], packet->payload + x, data_len);
              flow->metadata.protos.dns.is_rsp_addr_ipv6[flow->metadata.protos.dns.num_rsp_addr] = (data_len == 16) ? 1 : 0;
              flow->metadata.protos.dns.rsp_addr_ttl[flow->metadata.protos.dns.num_rsp_addr] = ttl;

              if(ndpi_struct->cfg.address_cache_size)
                ndpi_cache_address(ndpi_struct,
                                   flow->metadata.protos.dns.rsp_addr[flow->metadata.protos.dns.num_rsp_addr],
                                   flow->metadata.host_server_name,
                                   packet->current_time_ms/1000,
                                   flow->metadata.protos.dns.rsp_addr_ttl[flow->metadata.protos.dns.num_rsp_addr]);
		...
		ndpi_cache_hostname_ip(ndpi_struct,
              ...
              ++flow->metadata.protos.dns.num_rsp_addr;
            }
  ...
  if(dns_header->additional_rrs > 0) {
    ...
    for(num = 0; num < dns_header->additional_rrs; num++) {
      ...
      if(rsp_type == 41 /* OPT */) {
        ...
        flow->metadata.protos.dns.edns0_udp_payload_size = ntohs(*((u_int16_t*)&packet->payload[x])); /* EDNS(0) */

#ifdef DNS_DEBUG
        printf("[DNS] [response] edns0_udp_payload_size: %u\n", flow->metadata.protos.dns.edns0_udp_payload_size);
#endif
        x += 6;
...
#endif
              memcpy(flow->metadata.protos.dns.geolocation_iata_code, opt + 6,
                     ndpi_min(opt_len - 6, (int)sizeof(flow->metadata.protos.dns.geolocation_iata_code) - 1));
            }
          }
...
static int is_valid_dns(struct ndpi_detection_module_struct *ndpi_struct,
  ...
  if(*is_query) {
    ...
    if(((dns_header->num_queries > 0 && dns_header->num_queries <= NDPI_MAX_DNS_REQUESTS) || /* Don't assume that num_queries must be zero */
        (checkDNSSubprotocol(ndpi_struct, ntohs(flow->core.c_port), ntohs(flow->core.s_port)) == NDPI_PROTOCOL_MDNS && dns_header->num_queries == 0)) &&
				       ...
				       struct ndpi_flow_struct *flow)
{
  ...
  if(flow->core.detected_protocol_stack[0] == NDPI_PROTOCOL_LLMNR ||
     flow->core.detected_protocol_stack[1] == NDPI_PROTOCOL_LLMNR)
    return;

  flow->core.max_extra_packets_to_check = ndpi_struct->cfg.dns_max_packets_extra_dissection;
  flow->core.extra_packets_func = search_dns_again;
}
			   ...
			   struct ndpi_flow_struct *flow)
{
  ...
  if(flow->core.dns_tcp_reasm != NULL) {
    reasm = &flow->core.dns_tcp_reasm->dir[packet->packet_direction];
  ...
  for(offset = 0; offset + 2 <= original_payload_len; ) {
    ...
    search_dns_tcp_udp(ndpi_struct, flow);
    processed = 1;
  ...
  if(flow->core.dns_tcp_reasm == NULL) {
    flow->core.dns_tcp_reasm = ndpi_calloc(1, sizeof(struct ndpi_dns_tcp_reasm_state));
    if(flow->core.dns_tcp_reasm == NULL)
      return -1;
  }
...
static int keep_extra_dissection(struct ndpi_flow_struct *flow)
{
  ...
  return !(!flow->metadata.protos.dns.is_query && flow->metadata.protos.dns.num_answers != 0);
}
...
static int process_hostname(struct ndpi_detection_module_struct *ndpi_struct,
  ...
  proto->master_protocol = checkDNSSubprotocol(ndpi_struct, ntohs(flow->core.c_port), ntohs(flow->core.s_port));
  proto->app_protocol = flow->core.detected_protocol_stack[1] != NDPI_PROTOCOL_UNKNOWN ? flow->core.detected_protocol_stack[0] : NDPI_PROTOCOL_UNKNOWN;
      ...
      if((first_element_len > 48) && (!is_mdns)) {
        ...
        */
        ...
        if(ends_with(ndpi_struct, _hostname, "multi.surbl.org")
          ...
          ndpi_set_risk(ndpi_struct, &flow->core, NDPI_DNS_SUSPICIOUS_TRAFFIC, "Long DNS host name");
      }
    }
  }

  if(strlen(flow->metadata.host_server_name) > 0) {
    ...
    ndpi_check_dga_name(ndpi_struct, flow, flow->metadata.host_server_name, 1, 0, proto->app_protocol != NDPI_PROTOCOL_UNKNOWN);
  }

  return 0;
}

static void search_dns_tcp_udp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow) {
  struct ndpi_packet_struct *packet = &ndpi_struct->packet;
  ...
  if(!is_valid_dns(ndpi_struct, flow, &dns_header, payload_offset, &is_query)) {
    ...
    if(flow->core.extra_packets_func == NULL) {
      ...
      ndpi_set_risk(ndpi_struct, &flow->core, NDPI_MALFORMED_PACKET, "Invalid DNS Header");
    }
    return;
  }
  ...
  if(is_query) {
    flow->metadata.protos.dns.is_query = 1;
    flow->metadata.protos.dns.transaction_id = dns_header.tr_id;
    ...
    flow->metadata.protos.dns.is_query = 0;
    flow->metadata.protos.dns.transaction_id = dns_header.tr_id;
    flow->metadata.protos.dns.reply_code = dns_header.flags & 0x0F;
    flow->metadata.protos.dns.num_queries = dns_header.num_queries;
    flow->metadata.protos.dns.num_answers = dns_header.num_answers + dns_header.authority_rrs + dns_header.additional_rrs;

    if(flow->metadata.protos.dns.reply_code != 0) {
      if(is_flowrisk_info_enabled(ndpi_struct, NDPI_ERROR_CODE_DETECTED)) {
        char str[32], buf[16];

        snprintf(str, sizeof(str), "DNS Error Code %s",
                 dns_error_code2string(flow->metadata.protos.dns.reply_code, buf, sizeof(buf)));
        ndpi_set_risk(ndpi_struct, &flow->core, NDPI_ERROR_CODE_DETECTED, str);
      } else {
        ndpi_set_risk(ndpi_struct, &flow->core, NDPI_ERROR_CODE_DETECTED, NULL);
      }
    } else {
      if(ndpi_isset_risk(&flow->core, NDPI_SUSPICIOUS_DGA_DOMAIN)) {
        ndpi_set_risk(ndpi_struct, &flow->core, NDPI_RISKY_DOMAIN, "DGA Name Query with no Error Code");
      }
    }
    ...
    if(proto.master_protocol == NDPI_PROTOCOL_DNS &&
       ...
       packet->payload_packet_len > flow->metadata.protos.dns.edns0_udp_payload_size) {
      if(is_flowrisk_info_enabled(ndpi_struct, NDPI_DNS_LARGE_PACKET)) {
        ...
        ndpi_set_risk(ndpi_struct, &flow->core, NDPI_DNS_LARGE_PACKET, str);
      } else {
        ndpi_set_risk(ndpi_struct, &flow->core, NDPI_DNS_LARGE_PACKET, NULL);
      }
    }

    NDPI_LOG_DBG2(ndpi_struct, "Response: [num_queries=%d][num_answers=%d][reply_code=%u][rsp_type=%u][metadata.host_server_name=%s]\n",
                  flow->metadata.protos.dns.num_queries, flow->metadata.protos.dns.num_answers,
                  flow->metadata.protos.dns.reply_code, flow->metadata.protos.dns.rsp_type, flow->metadata.host_server_name);
  }
  ...
  flow->core.category = NDPI_PROTOCOL_CATEGORY_NETWORK;
  flow->core.breed = NDPI_PROTOCOL_ACCEPTABLE;
  ...
  if(!flow->core.extra_packets_func &&
     ...
     flow->core.detected_protocol_stack[0] != NDPI_PROTOCOL_LLMNR &&
     flow->core.detected_protocol_stack[1] != NDPI_PROTOCOL_LLMNR) {
  ...
  if((flow->core.detected_protocol_stack[0] == NDPI_PROTOCOL_DNS)
     || (flow->core.detected_protocol_stack[1] == NDPI_PROTOCOL_DNS)) {
	...
	ndpi_set_risk(ndpi_struct, &flow->core, NDPI_DNS_FRAGMENTED, NULL);
      }
...
void ndpi_search_dns_tcp_udp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow) {
  struct ndpi_packet_struct *packet = &ndpi_struct->packet;
  ...
  if(s_port == DNS_PORT || d_port == DNS_PORT ||
    ...
    /* Standard case, keep going */
            ...
            flow->metadata.l4.udp.eaq_pkt_id == 0 && flow->metadata.l4.udp.eaq_sequence == 0) {
